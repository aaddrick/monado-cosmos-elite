...
static const char VIVE_COSMOS_ELITE_PRODUCT_STRING[] = "VIVE COSMOS";
...
static const char VIVE_COSMOS_ELITE_MANUFACTURER_STRING[] = "HTC Corporation";
...
static void
init_vive_cosmos_elite(struct xrt_prober *xp,
               struct xrt_prober_device *dev,
               struct xrt_prober_device **devices,
               size_t device_count,
               enum u_logging_level log_level,
               ...
               struct vive_device **out_vdev)
{
	...
	if (!u_prober_match_string(xp, dev, XRT_PROBER_STRING_MANUFACTURER, VIVE_COSMOS_ELITE_MANUFACTURER_STRING) ||
	    !u_prober_match_string(xp, dev, XRT_PROBER_STRING_PRODUCT, VIVE_COSMOS_ELITE_PRODUCT_STRING)) {
		U_LOG_D("Vive Cosmos Elite manufacturer string did not match.");
		return;
	}
	...
	for (uint32_t i = 0; i < device_count; i++) {
		...
		int result = xrt_prober_open_hid_interface(xp, d, 0, &sensors_dev);
		if (result != 0) {
			U_LOG_E("Could not open Vive Cosmos Elite sensors device.");
			return;
		}
	...
	if (sensors_dev == NULL) {
		U_LOG_E("Could not find Vive Cosmos Elite sensors device.");
		return;
	}
	...
	struct vive_device *d =
	    vive_device_create(mainboard_dev, sensors_dev, watchman_dev, VIVE_VARIANT_COSMOS_ELITE, tstatus, vs);
	if (d == NULL) {
		free(sensors_dev);
		free(mainboard_dev);
		return;
           ...
           struct xrt_device **out_xdev)
{
	...
	case VIVE_COSMOS_ELITE_MAINBOARD_PID: {
		init_vive_cosmos_elite(xp, dev, devices, device_count, log_level, tstatus, vs, &vdev);
		break;
	}
